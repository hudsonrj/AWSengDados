...
# 030
# Fluxo de execução da transferência de arquivos da zona de processamento para a raw zone 
        ...
        if (caminho in arquivo_zip_base.key):
            ...
            if ((not arquivo_a_processar.lower().endswith('.zip')) and (not arquivo_a_processar.lower().endswith('.7z'))):
                ...
                if (cp.ACCESS_TYPE == "DB"):
                    copiar_arquivo(    arquivo_a_processar, bd, caminho, s3, pasta_arquivos, arquivo_zip_base.key, pasta_raw, arquivo_destino, origem, caminho_nome)
                else:
                    copiar_arquivo_csv(arquivo_a_processar, bd, caminho, s3, pasta_arquivos, arquivo_zip_base.key, pasta_raw, arquivo_destino, origem, caminho_nome)
                # end if
            else:
...
def copiar_arquivo(arquivo_a_processar, bd, caminho, s3, pasta_origem, arquivo_origem, pasta_destino, arquivo_destino, origem, caminho_nome):
    #Configurações para processamento
    ...
    try:
        ...
        hash_pesquisa = "P04_" + hash_arquivo + "_" + caminho_nome

        linhas = zf.getDBRows(db, "file_hash_control", "hash", hash_pesquisa)
        if (linhas == 0):
            aNomeArquivo = os.path.splitext(arquivo_a_processar)
    
            arquivo_a_processar_renomeado = aNomeArquivo[0] + "_" + caminho_nome + "_" + zf.getDateTime() + "_" + zf.getRandomNumber(10) + aNomeArquivo[1]
...
# Função para validação dos arquivos .zip
def copiar_arquivo_csv(arquivo_a_processar, bd, caminho, s3, pasta_origem, arquivo_origem, pasta_destino, arquivo_destino, origem, caminho_nome):
    #Configurações para processamento
    pasta_processamento = cp.PROCESSING_FOLDER
    ...
    try:
        ...
        if (linhas == 0):
            ...
            arquivo_destino_renomeado = arquivo_destino.replace(arquivo_a_processar, arquivo_a_processar_renomeado)
            s3.Object(pasta_destino, arquivo_destino_renomeado).copy_from(CopySource={'Bucket': pasta_origem, 'Key': caminho_arquivo_processamento})
            zf.append_to_csv_s3(cp.LANDING_FOLDER, cp.FILE_HASH_NAME, [(hash_pesquisa, zf.getDateTime("%Y-%m-%d %H:%M:%S"), arquivo_a_processar, arquivo_a_processar_renomeado, origem, "PROCESSADO", "")])
        # end if
        s3.Object(pasta_origem, caminho_arquivo_processamento).delete()
    except Exception as e: 
        erro_arquivo = str(e)
...
def main():
    print("\nTransferencia para raw zone - inicio "  + zf.getDateTime("%Y-%m-%d %H:%M:%S"))
    ...
    # end for
    print("Transferencia para raw zone - fim    " + zf.getDateTime("%Y-%m-%d %H:%M:%S"))
# end def
